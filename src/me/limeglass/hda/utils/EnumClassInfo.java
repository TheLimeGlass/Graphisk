...
public class EnumClassInfo<E extends Enum<E>> {
	...
	public ClassInfo<E> register() {
		if (Classes.getExactClassInfo(enumType) == null) {
			if (HolographicDisplaysAddon.getInstance().getConfig().getBoolean("UseEnglishEnums")) {
				...
				HolographicDisplaysAddon.getConfiguration("syntax").set("Syntax.Enums." + enumType.getSimpleName() + ".enums", enumType.getEnumConstants());
			}
	...
	private void enumDebug(String message, String codeName, Class<?> clazz) {
		if (codeName.equalsIgnoreCase(HolographicDisplaysAddon.getInstance().getConfig().getString("syntaxDebug"))) HolographicDisplaysAddon.debugMessage("&eRegistered syntaxDebug found enum classinfo: " + codeName + " (" + clazz.getCanonicalName() + ")");
		else HolographicDisplaysAddon.debugMessage("&5Registered Enum '" + codeName + "' with return class " + enumType.getName());
		HolographicDisplaysAddon.getConfiguration("syntax").set("Syntax.Enums." + clazz.getSimpleName() + ".user", user);
